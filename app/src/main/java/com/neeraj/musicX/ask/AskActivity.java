...
public class AskActivity extends AppCompatActivity {
    ...
    private void init(Bundle state) {
        if (state != null) {
            permissions = state.getStringArray(com.neeraj.musicX.ask.Constants.PERMISSIONS);
            rationaleMessages = state.getStringArray(com.neeraj.musicX.ask.Constants.RATIONAL_MESSAGES);
            requestId = state.getInt(com.neeraj.musicX.ask.Constants.REQUEST_ID);
        } else {
            Intent intent = getIntent();
            permissions = intent.getStringArrayExtra(com.neeraj.musicX.ask.Constants.PERMISSIONS);
            requestId = intent.getExtras().getInt(com.neeraj.musicX.ask.Constants.REQUEST_ID, 0);
        }
    }

    @Override
    public void onSaveInstanceState(Bundle state) {
        state.putStringArray(com.neeraj.musicX.ask.Constants.PERMISSIONS, permissions);
        state.putStringArray(com.neeraj.musicX.ask.Constants.RATIONAL_MESSAGES, rationaleMessages);
        state.putInt(com.neeraj.musicX.ask.Constants.REQUEST_ID, requestId);
    }
    ...
    private void broadcast(String[] permissions, int[] grantResults) {
        if (grantResults.length > 0) {
            ...
            intent.putExtra(com.neeraj.musicX.ask.Constants.PERMISSIONS, permissions);
            intent.putExtra(com.neeraj.musicX.ask.Constants.GRANT_RESULTS, grantResults);
            intent.putExtra(com.neeraj.musicX.ask.Constants.REQUEST_ID, requestId);
